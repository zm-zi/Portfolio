document.addEventListener('DOMContentLoaded', () => {
  ...
  if (assetGallery) {
    ...
    // Focus trap for asset gallery
    assetGallery.addEventListener('keydown', (e) => {
      if (e.key === 'Tab' && assetGallery.classList.contains('asset-gallery--open')) {
        const focusable = assetGallery.querySelectorAll('button, [href], [tabindex]:not([tabindex="-1"])');
        if (focusable.length === 0) return;
        const first = focusable[0];
        const last = focusable[focusable.length - 1];
        if (e.shiftKey) {
